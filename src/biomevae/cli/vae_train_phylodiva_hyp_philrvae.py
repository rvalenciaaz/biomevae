"""Training script for :class:`PhyloDIVAHyperbolicPhILRVAE`.

Mirrors :mod:`biomevae.cli.vae_train_diva_hyp_philrvae` plus the optional
PhyloDIVA regularisers: gradient-reversed study critic on z_y (with
class-context conditioning), Deep CORAL on z_x, and a shift-invariant
PhILR-coord smoothness penalty on ``coord_mu``.
"""
...
# Default Optuna search space for PhyloDIVA-Hyperbolic-PhILR-VAE: shared
# DIVA defaults + Poincaré curvature + PhyloDIVA regularisation knobs.
DEFAULT_PHYLODIVA_HYP_PHILRVAE_SEARCH_SPACE: Dict[str, Any] = {
    **DEFAULT_DIVA_SEARCH_SPACE,
...
def build_parser() -> argparse.ArgumentParser:
    ap = argparse.ArgumentParser("biomevae-train-phylodiva-hyp-philrvae")
    ...
    ap.add_argument("--lambda-philr-smooth", type=float, default=0.0)
    ap.add_argument("--grl-lambda", type=float, default=1.0)
    ap.add_argument("--no-critic-condition-on-class", action="store_true")
...
def _train(
    ...
    *,
    ...
    """Run one PhyloDIVA-Hyperbolic-PhILR-VAE training pass."""
    outdir = Path(outdir)
    ...
    cfg = {
        "model_type": "phylodiva-hyp-philrvae",
...
def main(argv=None) -> None:
    ...
    if args.optuna:
        run_diva_optuna(
            ...
            default_search_space=DEFAULT_PHYLODIVA_HYP_PHILRVAE_SEARCH_SPACE,
        )
        return
