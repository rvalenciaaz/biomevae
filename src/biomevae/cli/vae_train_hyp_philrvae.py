"""Training script for :class:`HyperbolicPhILRVAE`.

Identical I/O contract to :mod:`biomevae.cli.vae_train_philrvae` plus a
``--curvature`` flag for the Poincaré-ball latent. The decoder uses
``logmap0`` to project ball points back to tangent space before any
Euclidean Linear layer (audit D2).
"""
...
from biomevae.cli.vae_train_philrvae import (
    DEFAULT_PHILRVAE_SEARCH_SPACE,
    LIKELIHOOD_CHOICES,
)
...
# Hyperbolic backbone adds the Poincaré-ball ``curvature`` knob on top of
# the Euclidean PhILR-VAE search space (latent_dim, lr, dropout, etc.).
DEFAULT_HYP_PHILRVAE_SEARCH_SPACE: Dict[str, Dict[str, Any]] = {
    **copy.deepcopy(DEFAULT_PHILRVAE_SEARCH_SPACE),
    "curvature": {"method": "suggest_float", "low": 0.25, "high": 4.0, "log": True},
}


def build_parser() -> argparse.ArgumentParser:
    ap = argparse.ArgumentParser("biomevae-train-hyp-philrvae")
...
def _train(
    ...
    *,
    ...
    """Run one Hyperbolic-PhILR-VAE training pass."""
    outdir = Path(outdir)
    ...
    for ep in range(1, args.epochs + 1):
        ...
        if improved:
            ...
            if args.early_stop > 0 and no_improve >= args.early_stop:
                ...
                break
        if verbose:
            print(f"ep {ep:03d} | beta={beta:.3f} | val_recon={val_nll:.4f} kl={row['val_kl']:.4f}")
    ...
    cfg = {
        "model_type": "hyperbolic-philrvae",
    ...
    with torch.no_grad():
        for (xb,) in loader:
            ...
            dec = model.decode(model.manifold.projx(model.manifold.expmap0(mu)))
...
def main(argv=None) -> None:
    ...
    if args.optuna:
        run_diva_optuna(
            ...
            default_search_space=DEFAULT_HYP_PHILRVAE_SEARCH_SPACE,
        )
        return
