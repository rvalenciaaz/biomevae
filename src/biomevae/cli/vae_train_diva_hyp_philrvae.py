"""Training script for :class:`DIVAHyperbolicPhILRVAE`.

Mirrors :mod:`biomevae.cli.vae_train_diva_tree_dtm_vae` exactly, but the
backbone is the hyperbolic compositional PhILR-VAE.
"""
...
# Default Optuna search space for the Hyperbolic DIVA-PhILR backbone.
# Adds the Poincaré-ball ``curvature`` knob and the Dirichlet-tree
# concentration prior on top of the shared DIVA defaults.
DEFAULT_DIVA_HYP_PHILRVAE_SEARCH_SPACE: Dict[str, Any] = {
    **DEFAULT_DIVA_SEARCH_SPACE,
...
def build_parser() -> argparse.ArgumentParser:
    ap = argparse.ArgumentParser("biomevae-train-diva-hyp-philrvae")
...
def _build_dataset(args) -> Tuple[DIVADatasetTensors, torch.Tensor, list, object]:
    ...
    if missing:
        print(
            f"[align] WARNING: {len(missing)} taxonomy leaves missing from merged "
            f"SGB table (filled with zero). Example: {missing[:5]}"
        )
    ...
    ds = DIVADatasetTensors(
        x_log=torch.from_numpy(np.log(X_leaf + 1.0).astype(np.float32)),
    ...
    return ds, torch.from_numpy(X_leaf), leaf_names, taxg
...
def _train(
    ...
    *,
    ...
    """Run one DIVA-Hyperbolic-PhILR-VAE training pass."""
    outdir = Path(outdir)
    ...
    for ep in range(1, args.epochs + 1):
        ...
        if verbose:
            print(
                ...
                f"kl={va.get('kl_total', 0):.3f} ce_y={va.get('ce_y', 0):.3f}"
            )
...
def main(argv=None) -> None:
    ...
    if args.optuna:
        run_diva_optuna(
            ...
            default_search_space=DEFAULT_DIVA_HYP_PHILRVAE_SEARCH_SPACE,
        )
        return
