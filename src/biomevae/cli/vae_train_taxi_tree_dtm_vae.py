"""Training script for TAXI-DIVA-Tree-DTM-VAE.

Mirrors :mod:`biomevae.cli.vae_train_phylodiva_tree_dtm_vae` but instantiates
:class:`biomevae.models.taxi_treedtmvae.TAXIDIVATreeDTMVAE` — the taxonomy-
protected conditional-invariance variant.

Differences vs. PhyloDIVA-Tree-DTM:

* The protected taxonomy/class latent ``z_tau`` (== DIVA ``z_y``) is **never**
  adversarially scrubbed.  Only the residual predictive latent ``z_rho``
  (== DIVA ``z_x``) is passed through the GRL of the conditional study
  critic.
* Class-conditional CORAL replaces marginal CORAL: covariances are matched
  within each phenotype class so cross-study alignment does not collapse
  legitimate disease-induced taxonomic shifts.
* A cross-covariance orthogonality penalty between ``z_tau`` and ``z_rho``
  is available (``--lambda-orth``) to discourage redundancy.
* An auxiliary CE on ``z_tau`` alone (``--lambda-tau-aux``) optionally
  pins the protected channel to the phenotype signal so the residual
  channel does not absorb it.
"""
...
# TAXI uses ``latent_tau`` / ``latent_rho`` rather than DIVA's
# ``latent_y`` / ``latent_x``, so the default DIVA Optuna search space
# can't be reused as-is — the search-space keys must match the argparse
# attribute names that Optuna writes back onto ``args``.  This space
# mirrors the DIVA defaults under TAXI's naming, plus the conditional-
# invariance λs that distinguish TAXI from PhyloDIVA.
DEFAULT_TAXI_TREE_DTM_SEARCH_SPACE: Dict[str, Any] = {
    ...
    # TAXI-specific conditional-invariance penalties.
    "lambda_cond_critic": {
...
def build_parser() -> argparse.ArgumentParser:
    ap = argparse.ArgumentParser("biomevae-train-taxi-tree-dtm")
    ...
    ap.add_argument(
        "--lambda-tree-smooth", type=float, default=0.0,
        help="Weight on the centred adjacent edge-logit smoothness penalty.",
    )
    ap.add_argument(
...
def _train(
    ...
    *,
    ...
    """Run one TAXI-DIVA-Tree-DTM-VAE training pass."""
    outdir = Path(outdir)
...
def main(argv=None) -> None:
    ...
    if args.optuna:
        run_diva_optuna(
            ...
            default_search_space=DEFAULT_TAXI_TREE_DTM_SEARCH_SPACE,
        )
        return
